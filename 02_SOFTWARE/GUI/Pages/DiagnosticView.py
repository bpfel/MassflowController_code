...
class DiagnosticView(tk.Frame):
    def __init__(self, parent, figures, setup: Setup):
        ...
        #canvas_flow = FigureCanvasTkAgg(figures["flows"], plotting_section)
        #canvas_flow.draw()
        #canvas_flow.draw()
        #canvas_flow.get_tk_widget().grid(column=1, row=0, sticky="sewn")

        canvas_temp = FigureCanvasTkAgg(figures["delta_temperatures"], plotting_section)
        canvas_temp.draw()
        canvas_temp.get_tk_widget().grid(column=0, row=0, sticky="sewn")

        canvas_d_temp = FigureCanvasTkAgg(figures["temperatures"], plotting_section)
        canvas_d_temp.draw()
        canvas_d_temp.get_tk_widget().grid(column=0, row=1, sticky="sewn")

        canvas_pwm = FigureCanvasTkAgg(figures["pwm"], plotting_section)
        canvas_pwm.draw()
